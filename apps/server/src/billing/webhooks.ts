...
export const handleCustomerStateChanged =
  (deps: { logger: Logger }) =>
    (payload: WebhookCustomerStateChangedPayload):
      Promise<Result<{ processed: true }, { message: string; cause?: unknown }>> =>
      Result.tryPromise({
        try: async () => {
          const { logger } = deps;
          ...
          // No local state to update - we query Polar directly when needed
          // This is where you could trigger cache invalidation if you add caching later
...
export const handleSubscriptionActive =
  (deps: { logger: Logger }) =>
    (payload: WebhookSubscriptionActivePayload):
      Promise<Result<{ activated: true }, { message: string; cause?: unknown }>> =>
      Result.tryPromise({
        try: async () => {
          const { logger } = deps;
          const data = payload.data as PayloadData;
          ...
          // Could trigger: welcome email, resource provisioning, analytics tracking
...
export const handleSubscriptionCanceled =
  (deps: { logger: Logger }) =>
    (payload: WebhookSubscriptionCanceledPayload):
      Promise<Result<{ canceled: true }, { message: string; cause?: unknown }>> =>
      Result.tryPromise({
        try: async () => {
          const { logger } = deps;
          const data = payload.data as PayloadData;
          ...
          // Could trigger: retention email, survey, etc.
    ...
    (payload: WebhookSubscriptionRevokedPayload):
      ...
      Result.tryPromise({
        try: async () => {
          ...
          // Could trigger: downgrade email, data export reminder, etc.
...
export const handleSubscriptionUncanceled =
  (deps: { logger: Logger }) =>
    (payload: WebhookSubscriptionUncanceledPayload):
      Promise<Result<{ uncanceled: true }, { message: string; cause?: unknown }>> =>
      Result.tryPromise({
        try: async () => {
          const { logger } = deps;
          const data = payload.data as PayloadData;
