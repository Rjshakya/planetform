...
import {
	type Benefits,
	type CustomerState,
	type FreePlanBenefits,
	PolarApiError,
	type ProPlanBenefits,
	type Subscription,
} from "./types";
...
export const hasActiveSubscription =
	...
	(params: {
		userId: string;
		productId: string;
	}): Promise<Result<boolean, PolarApiError>> =>
		Result.gen(async function* () {
			const state = yield* await getCustomerState(deps)({
				userId: params.userId,
			});

			const hasActive = state.activeSubscriptions.some(
				(sub: Subscription) =>
					sub.productId === params.productId && sub.status === "active",
			);
...
export const getPlanBenefits =
	...
	(params: { userId: string }): Promise<Result<Benefits, PolarApiError>> =>
		Result.gen(async function* () {
			const isPro = yield* await hasActiveSubscription({
			...
			if (isPro) {
				return Result.ok({
					...
					pro: true,
					maxWorkspaces: 9999,
					maxFormsPerWorkspace: 9999,
					maxResponsesPerMonth: 9999,
